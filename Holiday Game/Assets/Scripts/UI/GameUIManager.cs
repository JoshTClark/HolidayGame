    ...
    public void UpdateUI(GameManager.GameState state, Player player, GameManager gameManager)
    {
        ...
        switch (state)
        {
            ...
            case GameState.MainGame:
                ...
                // Updating player stat display
                currentLayout.dashText.text = player.currentDashes.ToString();
                currentLayout.xpBar.GetComponent<RectTransform>().anchorMax = new Vector2(player.GetPercentToNextLevel(), currentLayout.xpBar.GetComponent<RectTransform>().anchorMax.y);
                currentLayout.dashTimer.GetComponent<RectTransform>().anchorMax = new Vector2(1 - (player.dashCooldownTimer / player.DashCooldown), currentLayout.dashTimer.GetComponent<RectTransform>().anchorMax.y);
                currentLayout.playerLevel.text = "Level: " + player.Level;
                currentLayout.hpBar.rectTransform.anchorMax = new Vector2(player.GetPercentHealth(), currentLayout.hpBar.rectTransform.anchorMax.y);
                currentLayout.hpText.text = player.CurrentHP.ToString("0") + "/" + player.MaxHp.ToString("0");
                ...
                // Level up text
                if (player.waitingForLevels > 0) { currentLayout.levelUpText.gameObject.SetActive(true); }
                else { currentLayout.levelUpText.gameObject.SetActive(false); }
                break;
        }
    }
